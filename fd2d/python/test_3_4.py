...
def main():
    ...
    iz = np.zeros((nx, ny), dtype=np.float32)
    ...
    freq = np.array((50e6, 300e6, 700e6), dtype=np.float32)
    nf: int = len(freq)  # number of frequencies

    ft = ftrans(
        r_pt = np.zeros((nf, nx, ny), dtype=np.float32),
        i_pt = np.zeros((nf, nx, ny), dtype=np.float32),
        r_in = np.zeros(nf, dtype=np.float32),
        i_in = np.zeros(nf, dtype=np.float32),
    )
