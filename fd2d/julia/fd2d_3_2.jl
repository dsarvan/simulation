...
function surfaceplot(ns::Int, nx::Int, ny::Int, ez::Array{Float64})::Nothing
    ...
    plt.savefig("fd2d_surface_3_2.png")
end
...
function contourplot(ns::Int, nx::Int, ny::Int, ez::Array{Float64})::Nothing
    ...
    plt.savefig("fd2d_contour_3_2.png")
end
...
function dfield(t::Int32, nx::Int, ny::Int, pml::pmlayer, dz::Array{Float64}, hx::Array{Float64}, hy::Array{Float64})
    ...
    @views dz[2:ny,2:nx] .= pml.gy3[2:ny] .* pml.gx3[2:nx]' .* dz[2:ny,2:nx] .+ pml.gy2[2:ny] .* pml.gx2[2:nx]' .* 0.5 .* (hy[2:ny,2:nx] .- hy[1:ny-1,2:nx] .- hx[2:ny,2:nx] .+ hx[2:ny,1:nx-1])
    # put a sinusoidal source at a point that is offset four cells
    # from the center of the problem space in each direction
    dz[div(ny,2)-4,div(nx,2)-4] = sinusoidal(t, 0.01, 1500e6)
end


function efield(nx::Int, ny::Int, naz::Array{Float64}, dz::Array{Float64}, ez::Array{Float64})
    """ calculate the Ez field from Dz """
    @views ez[1:ny,1:nx] .= naz[1:ny,1:nx] .* dz[1:ny,1:nx]
end
...
function hfield(nx::Int, ny::Int, pml::pmlayer, ez::Array{Float64}, ihx::Array{Float64}, ihy::Array{Float64}, hx::Array{Float64}, hy::Array{Float64})
    ...
    curl_em = ez[1:ny-1,1:nx-1] - ez[1:ny-1,2:nx]
    curl_en = ez[1:ny-1,1:nx-1] - ez[2:ny,1:nx-1]
    @views ihx[1:ny-1,1:nx-1] .+= curl_em
    @views ihy[1:ny-1,1:nx-1] .+= curl_en
    @views hx[1:ny-1,1:nx-1] .= pml.fx3[1:nx-1]' .* hx[1:ny-1,1:nx-1] .+ pml.fx2[1:nx-1]' .* (0.5 .* curl_em .+ pml.fy1[1:ny-1] .* ihx[1:ny-1,1:nx-1])
    @views hy[1:ny-1,1:nx-1] .= pml.fy3[1:ny-1] .* hy[1:ny-1,1:nx-1] .- pml.fy2[1:ny-1] .* (0.5 .* curl_en .+ pml.fx1[1:nx-1]' .* ihy[1:ny-1,1:nx-1])
end
...
function main()
    ...
    ihx = zeros(Float64, (ny, nx))
    ihy = zeros(Float64, (ny, nx))
