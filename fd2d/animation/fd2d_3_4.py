...
def main():
    ...
    for t in np.arange(1, ns+1).astype(np.int32):
        ...
        for n, j in product(range(0, nf), range(npml-1, ny-npml+1)):
            m = (n,j); k = (n,nx//2-1,j)
        ...
        ampltdata.append(amplt.copy())

    ez = np.array(ezdata)
    amplt = np.array(ampltdata)

    # define the meta data for the movie
    fwriter = animation.writers["ffmpeg"]
    data = {"title": "Simulation of a plane wave striking dielectric material"}
    ...
    medium = np.stack([1.0/md.naz-1.0]*1, axis=2); levels = [0.50,1.50]
    pmlmsk = ((xv < npml) | (xv >= nx-npml) | (yv < npml) | (yv >= ny-npml))
    ax.plot_surface(xv, yv, ez[0], rstride=1, cstride=1, cmap="gray", alpha=0.5, lw=10/nx)
    ...
    with writer.saving(fig, "fd2d_surface_3_4.mp4", 300):
        for t in np.arange(1, ns+1).astype(np.int32):
            ax.clear()
            medium = np.stack([1.0/md.naz-1.0]*1, axis=2); levels = [0.50,1.50]
            pmlmsk = ((xv < npml) | (xv >= nx-npml) | (yv < npml) | (yv >= ny-npml))
            ax.plot_surface(xv, yv, ez[t], rstride=1, cstride=1, cmap="gray", alpha=0.5, lw=10/nx)
            ax.voxels(medium, color="y", edgecolor="k", shade=True, alpha=0.5, linewidths=1/nx)
            ax.contourf(xv, yv, pmlmsk, levels, offset=0, colors="k", alpha=0.40)
    ...
    fig.suptitle(r"FDTD simulation of plane wave striking dielectric material")
    yv, xv = np.meshgrid(range(ny), range(nx)); ezmax = np.abs(ez).max()
    medium = 1.0/md.naz-1.0; levels = np.linspace(-ezmax, ezmax, int(2/0.04))
    pmlmsk = ((xv < npml) | (xv >= nx-npml) | (yv < npml) | (yv >= ny-npml))
    ax.contour(xv, yv, ez[0], levels, cmap="gray", alpha=1.0, linewidths=1.0)
    ax.contourf(xv, yv, medium, [0.001,medium.max()], colors="y", alpha=0.7)
    ax.contourf(xv, yv, pmlmsk, levels=[0.5,1.5], colors="k", alpha=0.40)
    ax.set(xlim=(0, nx-1), ylim=(0, ny-1), aspect="equal")
    ax.set(xlabel=r"$x\;(cm)$", ylabel=r"$y\;(cm)$")
    plt.subplots_adjust(bottom=0.2, hspace=0.45)

    with writer.saving(fig, "fd2d_contour_3_4.mp4", 300):
        for t in np.arange(1, ns+1).astype(np.int32):
            ax.clear()
            medium = 1.0/md.naz-1.0; levels = np.linspace(-ezmax, ezmax, int(2/0.04))
            pmlmsk = ((xv < npml) | (xv >= nx-npml) | (yv < npml) | (yv >= ny-npml))
            ax.contour(xv, yv, ez[t], levels, cmap="gray", alpha=1.0, linewidths=1.0)
            ax.contourf(xv, yv, medium, [0.001,medium.max()], colors="y", alpha=0.7)
            ax.contourf(xv, yv, pmlmsk, levels=[0.5,1.5], colors="k", alpha=0.40)
            ax.set(xlim=(0, nx-1), ylim=(0, ny-1), aspect="equal")
            ax.set(xlabel=r"$x\;(cm)$", ylabel=r"$y\;(cm)$")
    ...
    axline, = ax.plot(range(-ny//2, ny//2), amplt[0,2], color="k", linewidth=1.0)
    ...
    axsigm = ax.text(0.86, 0.80, "", transform=ax.transAxes)
    plt.subplots_adjust(bottom=0.2, hspace=0.45)

    with writer.saving(fig, "fd2d_amplitude_3_4.mp4", 300):
        for t in np.arange(1, ns+1).astype(np.int32):
            axline.set_ydata(amplt[t,2])
