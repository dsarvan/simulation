...
ke = 200
ex = np.zeros(ke, dtype=np.complex_)
print(ex)
hy = np.zeros(ke, dtype=np.complex_)
...
epsilonr = complex(epsilon, -sigma/(2*np.pi*freq*epsz))

ca = np.ones(ke) + 1j
cb = 0.5 * np.ones(ke) + 1j

eaf = dt * sigma/(2*epsz*epsilonr)
ca[100:] = (1 - eaf)/(1 + eaf)
cb[100:] = 0.5/(epsilonr * (1 + eaf))
