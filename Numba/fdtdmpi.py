...
def plotting(time_step, ex, hy):
    ...
    plt.savefig("fdtdmpi.png")

def emfield(ke, kc, t0, spread, nsteps):

    ex = cp.zeros(ke).astype(np.float32)
    hy = cp.zeros(ke).astype(np.float32)

    assert hasattr(ex, '__cuda_array_interface__')
    assert hasattr(hy, '__cuda_array_interface__')
    ...
    for time_step in range(1, nsteps + 1):
        ...
        ex[100] = cp.exp(-0.5 * ((t0 - time_step) / spread) ** 2)
...
def main():
    ...
    #zz = cp.zeros(1).astype(np.float32)
    time_step, ex, hy = emfield(ke, kc, t0, spread, nsteps)
    plotting(time_step, ex, hy)

    #print(benchmark(emfield, (ke, kc, t0, spread, nsteps), n_repeat=20))
