...
function visualize(ns::Int, nx::Int, epsr::Float32, sigma::Float32, nax::Array{Float32}, ex::Array{Float32})::Nothing
    ...
    fig.suptitle("FDTD simulation of a pulse striking Debye dielectric material")
    ...
    plt.savefig("test_2_3.png", dpi=100)
end
...
function amplitude(ns::Int, nx::Int, epsr::Float32, sigma::Float32, nax::Array{Float32}, amp::Array{Float32})::Nothing
    ...
    ax.text(0.85, 0.80, "\$\\sigma\$ = $sigma \$S/m\$", transform=ax.transAxes)
    plt.subplots_adjust(bottom=0.2, hspace=0.45)
    plt.savefig("test_amp_2_3.png", dpi=100)
end
...
function exfield(nx::Int, md::medium, dx::Array{Float32}, ix::Array{Float32}, sx::Array{Float32}, ex::Array{Float32})
...
function dielectric(nx::Int, dt::Float32, chi::Float32, tau::Float32, epsr::Float32, sigma::Float32)::medium
    md = medium(
    ...
    md.nax[nx÷2+1:nx] .= 1/(epsr + sigma*dt/eps0 + chi*dt/tau)
    md.nbx[nx÷2+1:nx] .= sigma*dt/eps0
    md.ncx[nx÷2+1:nx] .= exp(-dt/tau)
    md.ndx[nx÷2+1:nx] .= chi*dt/tau
    return md
end
...
function main()
    ...
    chi::Float32 = 2.0  # relaxation susceptibility
    tau::Float32 = 0.001e-6  # relaxation time (s)
    epsr::Float32 = 2.0  # relative permittivity
    sigma::Float32 = 0.01  # conductivity (S/m)
    md::medium = dielectric(nx, dt, chi, tau, epsr, sigma)

    # frequency 50 MHz, 200 MHz, 500 MHz
    freq = Float32[50e6, 200e6, 500e6]
