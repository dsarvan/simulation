...
# Use two-point magnetic source at kc-1 and kc such that hy[kc-1] = -hy[kc]
...
def animate(Ex: np.ndarray, Hy: np.ndarray) -> None:
    ...
    with writer.saving(fig, "fd1d_1_4.mp4", dpi=300):
...
def simulate(ke: int, ex: np.ndarray, hy: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    for t in range(1, nsteps + 1):
        ...
        # put two-point magnetic source at kc-1 and kc
        hy[kc-1], hy[kc] = -hy[kc], gaussian(t, 40, 12)
